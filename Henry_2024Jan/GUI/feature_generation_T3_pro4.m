function feature_generation_T3_pro4()
...
for i = 1:n_pic
    ...
    %     fprintf('Loading Hyperspectral Data file: %s\n', fullfile(hs_data(i).folder, hs_data(i).name));
    ...
    nc = length(bwconncomp(imread(fullfile(mask_data(i).folder, mask_data(i).name))).PixelIdxList);
    ...
    for j = 1:nchannel
        ...
        for k = 1:nc
            ...
            data_pi_chj_ck = HS_i_j_flat(bwconncomp(imread(fullfile(mask_data(i).folder, mask_data(i).name))).PixelIdxList{k});
...
percentage = 0.4;
[feature_table_all, n_cell_tot] = feature_table_YL(n_feature, nchannel, n_pic, percentage, s_HAC_all, s_mask_all);
%% create the specific features based on the kidney cells paper (Table 4)
feature_table = zeros(n_cell_tot,10);
feature_table(:,1) = feature_table_all(:,1,7)./ feature_table_all(:,5,30); % No.1
feature_table(:,2) = feature_table_all(:,1,25)./ feature_table_all(:,5,3); % No.2
feature_table(:,3) = feature_table_all(:,1,2)./ feature_table_all(:,5,28);  % No.4
feature_table(:,4) = feature_table_all(:,7,26); % No.5
feature_table(:,5) = feature_table_all(:,1,32)./ feature_table_all(:,1,28);   % No.6
feature_table(:,6) = feature_table_all(:,1,22)./ feature_table_all(:,5,10);  % No.7
feature_table(:,7) = feature_table_all(:,1,8).* feature_table_all(:,1,22); % No.8
feature_table(:,8) = feature_table_all(:,1,19)./ feature_table_all(:,5,5);   % No.9
feature_table(:,9) = feature_table_all(:,1,14).* feature_table_all(:,1,17);  % No.10
% Add the label as the last column in the table
feature_table(:,10) = label;
end
...
feature_table = zeros(n_cell_tot,7);
feature_table(:,1) = feature_table_all(:,1,4)./ feature_table_all(:,1,24); % No.1
feature_table(:,2) = feature_table_all(:,1,20)./ feature_table_all(:,1,28); % No.2
feature_table(:,3) = feature_table_all(:,1,5)./ feature_table_all(:,5,21);  % No.3
feature_table(:,4) = feature_table_all(:,1,23)./ feature_table_all(:,5,18); % No.4
feature_table(:,5) = feature_table_all(:,1,35)./ feature_table_all(:,1,28);   % No.5
feature_table(:,6) = feature_table_all(:,1,7)./ feature_table_all(:,1,16);  % No.6
% Add the label as the last column in the table
feature_table(:,7) = label;
end
...
feature_table(:,3) = feature_table_all(:,1,5)./ feature_table_all(:,5,21);  % No.3
feature_table(:,4) = feature_table_all(:,1,23)./ feature_table_all(:,5,18); % No.4
feature_table(:,5) = feature_table_all(:,1,33)./ feature_table_all(:,1,28);   % No.5
feature_table(:,6) = feature_table_all(:,1,7)./ feature_table_all(:,1,16);  % No.6
