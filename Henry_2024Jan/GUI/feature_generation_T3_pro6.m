function feature_generation_T3_pro6()
...
% Callback function for load button
    function load_callback(~,~)
        ...
        % 检查是否生成了特征表和加载了文件名
        if exist('generate_feature_table', 'var') && ~isempty(generate_feature_table) && exist('s_HAC_all', 'var') && ~isempty(s_HAC_all)
            % 将特征表转换为元胞数组
            cell_feature_table = num2cell(generate_feature_table);
            
            % 获取文件名
            file_names = {s_HAC_all.name}';
            
            % 确保文件名数量与特征表行数匹配
            file_names = repmat(file_names, ceil(size(cell_feature_table, 1) / numel(file_names)), 1);
            file_names = file_names(1:size(cell_feature_table, 1));
            
            % 将文件名作为第一列插入特征表
            complete_table = [file_names, cell_feature_table];
            
            % 保存带有文件名的完整特征表
            writecell(complete_table, fullpath);
        else
            msgbox('Feature table or file names are empty or do not exist.', 'Save Error');
        end
...
for i = 1:n_pic
    ...
    se = strel('rectangle', [10, 10]); % Create a 5x5 rectangular structuring element
...
feature_table = zeros(n_cell_tot,15);
feature_table(:,1) = feature_table_all(:,5,2)./ feature_table_all(:,5,5); % No.1
feature_table(:,2) = feature_table_all(:,1,7)./ feature_table_all(:,1,8); % No.2
feature_table(:,3) = feature_table_all(:,5,12)./ feature_table_all(:,5,21);  % No.3
feature_table(:,4) = feature_table_all(:,5,21)./ feature_table_all(:,5,22); % No.4
feature_table(:,5) = feature_table_all(:,1,8)./ feature_table_all(:,5,2); % No.5
feature_table(:,6) = feature_table_all(:,5,31)./ feature_table_all(:,5,32); % No.6

percentage = 0.4;
[feature_table_all, n_cell_tot] = feature_table_YL(n_feature, nchannel, n_pic, percentage, s_HAC_all, s_mask_all);
feature_table(:,7) = feature_table_all(:,1,8)./ feature_table_all(:,5,21); % No.7
feature_table(:,8) = feature_table_all(:,1,5)./ feature_table_all(:,5,4); % No.8
feature_table(:,9) = feature_table_all(:,1,2)./ feature_table_all(:,5,29);  % No.9
feature_table(:,10) = feature_table_all(:,7,8); % No.10
feature_table(:,11) = feature_table_all(:,1,31)./ feature_table_all(:,1,28);   % No.11
feature_table(:,12) = feature_table_all(:,1,22)./ feature_table_all(:,5,12);  % No.12
feature_table(:,13) = feature_table_all(:,1,27).* feature_table_all(:,5,5); % No.13
feature_table(:,14) = feature_table_all(:,1,25)./ feature_table_all(:,1,31);   % No.14
% Add the label as the last column in the table
feature_table(:,15) = label;
...
feature_table_10_percent = zeros(n_cell_tot, 19);
feature_table_10_percent(:,1) = feature_table_all_10(:,5,2)./ feature_table_all_10(:,5,5);
feature_table_10_percent(:,2) = feature_table_all_10(:,1,7)./ feature_table_all_10(:,1,8);
feature_table_10_percent(:,3) = feature_table_all_10(:,5,12)./ feature_table_all_10(:,5,21);
feature_table_10_percent(:,4) = feature_table_all_10(:,5,21)./ feature_table_all_10(:,5,22);
feature_table_10_percent(:,5) = feature_table_all_10(:,1,8)./ feature_table_all_10(:,5,2);
feature_table_10_percent(:,6) = feature_table_all_10(:,5,31)./ feature_table_all_10(:,5,32);
feature_table_10_percent(:,7) = feature_table_all_10(:,5,3)./ feature_table_all_10(:,5,15);
feature_table_10_percent(:,8) = feature_table_all_10(:,5,26)./ feature_table_all_10(:,5,27);
feature_table_10_percent(:,9) = feature_table_all_10(:,5,14)./ feature_table_all_10(:,5,22);
feature_table_10_percent(:,10) = feature_table_all_10(:,5,24)./ feature_table_all_10(:,5,28);
feature_table_10_percent(:,11) = feature_table_all_10(:,5,4)./ feature_table_all_10(:,5,18);
feature_table_10_percent(:,12) = feature_table_all_10(:,5,23)./ feature_table_all_10(:,5,30);
feature_table_10_percent(:,13) = feature_table_all_10(:,5,15)./ feature_table_all_10(:,5,34);
feature_table_10_percent(:,14) = feature_table_all_10(:,1,18)./ feature_table_all_10(:,5,3);
feature_table_10_percent(:,15) = feature_table_all_10(:,5,26)./ feature_table_all_10(:,5,23);
feature_table_10_percent(:,16) = feature_table_all_10(:,5,5)./ feature_table_all_10(:,5,23);
feature_table_10_percent(:,17) = feature_table_all_10(:,5,14)./ feature_table_all_10(:,5,23);
feature_table_10_percent(:,18) = feature_table_all_10(:,5,31)./ feature_table_all_10(:,5,18);
feature_table_10_percent(:,19) = feature_table_all_10(:,5,26)./ feature_table_all_10(:,5,21);
