function hyperspec_gui()
    ...
    fig = figure('Name','Data Loader','MenuBar','none','ToolBar','none','Position',[300 300 600 300]);

    hs_browse_btn = uicontrol('Style','pushbutton','String','Browse Hyperspectral Data','Position',[10 260 180 30],'Callback',@hs_browse_callback);
    hs_folder_txt = uicontrol('Style','text','Position',[200 260 180 50],'HorizontalAlignment','left');

    m_browse_btn = uicontrol('Style','pushbutton','String','Browse Mask Data','Position',[10 200 180 30],'Callback',@m_browse_callback);
    m_folder_txt = uicontrol('Style','text','Position',[200 200 180 50],'HorizontalAlignment','left');
    ...
    % Create a window to display nchannel variable
    channel_window = uicontrol('Style','text','String','Number of Channels: ','Position',[10 20 150 20]);
    channel_txt = uicontrol('Style','text','Position',[160 25 50 15],'HorizontalAlignment','left');
    ...
    function load_callback(~,~)
        ...
        set(channel_txt, 'String', num2str(nchannel)); % Update the channel text box
